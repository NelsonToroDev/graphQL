...
module.exports = {
  ...
  editCourse: async (root, { _id, input }) => {
    ...
    try {
      ...
      course = await db.collection('Courses').findOne({ _id: new ObjectId(_id) })
    } catch (error) {
      errorHandler(error)
    }
  ...
  deleteCourse: async (root, { _id }) => {
    ...
    try {
      db = await connectDB()
      await db.collection('Courses').deleteOne({ _id: new ObjectId(_id) })
    } catch (error) {
      errorHandler(error)
    }
  ...
  addPerson: async (root, { courseId, personId }) => {
    ...
    try {
      ...
      await db.collection('Courses').updateOne(
        ...
        { $addToSet: { people: new ObjectId(personId) } }
      )
  ...
  editPerson: async (root, { _id, input }) => {
    ...
    try {
      ...
      person = await db.collection('Students').findOne({ _id: new ObjectId(_id) })
    } catch (error) {
      errorHandler(error)
    }
  ...
  deletePerson: async (root, { _id }) => {
    ...
    try {
      db = await connectDB()
      await db.collection('Students').deleteOne({ _id: new ObjectId(_id) })
    } catch (error) {
      errorHandler(error)
    }
